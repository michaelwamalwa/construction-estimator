...
setDocument = Sizzle.setDocument = function( node ) {
	...
	// className
	Expr.find[ "className" ] = support.getElementsByclass && function( class, context ) {
...
Expr = Sizzle.selectors = {
	...
	filter: {
		...
		"className": function( class ) {
			var pattern = classNameCache[ class + " " ];

			return pattern ||
				( pattern = new RegExp( "(^|" + whitespace +
					")" + class + "(" + whitespace + "|$)" ) ) && classNameCache(
						class, function( elem ) {
							return pattern.test(
...
jQuery.fn.extend( {
	...
	toggleclassName: function( value, stateVal ) {
		...
		return this.each( function() {
			...
			if ( isValidValue ) {
				...
				i = 0;
				self = jQuery( this );
				classs = classNameesToArray( value );

				while ( ( class = classs[ i++ ] ) ) {

					// Check each class given, space separated list
					if ( self.hasclassName( class ) ) {
						self.removeclassName( class );
					} else {
						self.addclassName( class );
					}
				}
	...
	hasclassName: function( selector ) {
		...
		while ( ( elem = this[ i++ ] ) ) {
			if ( elem.nodeType === 1 &&
				( " " + stripAndCollapse( getclassName( elem ) ) + " " ).indexOf( class ) > -1 ) {
				return true;
			}
		}
